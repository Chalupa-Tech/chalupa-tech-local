...
2. **PR 2** — Resize `talos-cp-2` (192.168.1.228, VMID 304) to 4 GB **and** introduce a `pulumi.DependsOn` chain across all three CPs (`talos-cp` → `talos-cp-2` → `talos-cp-3`). The chain is the long-term structural fix: it serializes Pulumi's stop / update / start across the CPs for any future restart-triggering field change (memory, CPU, disk), so a single apply can no longer cold-restart the quorum simultaneously. Etcd quorum maintained by `talos-cp` + `talos-cp-3` during this PR's resize.
