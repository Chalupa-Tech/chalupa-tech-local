...
## ⚠️ POST-IMPLEMENTATION AMENDMENTS (2026-05-12)

The plan below was written before implementation. Several assumptions turned out wrong; the corrections that actually shipped are summarized here so future-me reading this plan doesn't repeat the mistakes:

1. **Service FQDN was wrong.** The plan used `vlsingle-vlsingle-chalupa.vl-system.svc.cluster.local:9428` (the vm-operator pattern). The `victoria-logs-single` chart is a plain Helm chart, so the actual service is `vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428` (release name + chart name + component). All occurrences have been corrected throughout this document.

2. **VL has no Loki query API — only Loki ingestion.** The plan's decision (e) (Grafana's built-in Loki datasource) was based on the false assumption that VL serves Loki query endpoints. Verified live: every `/loki/api/v1/*` and `/select/loki/api/v1/*` path on VL v1.50.0 returns HTTP 400 `"unsupported path"`. Only `/insert/loki/api/v1/push` (ingest) and `/select/logsql/*` (native LogsQL queries) work. **The shipping implementation uses VictoriaMetrics's official Grafana plugin (`victoriametrics-logs-datasource`)** installed via the chart's `plugins:` values list. Queries are written in native LogsQL syntax (e.g., `namespace:argocd AND _msg:error`), not Loki's LogQL.

3. **Vector loki sink needs `/insert` suffix on the endpoint.** Vector appends `/loki/api/v1/push` to whatever endpoint is configured, so the endpoint URL must end in `/insert` for vector's POST to land at `/insert/loki/api/v1/push` (the path VL accepts). Without it, vector gets HTTP 400 on every push.

4. **VRL `merge()` is fallible; needs `merge!()` here.** VL needs object-form merging; the type system can't statically prove `structured` is an object even though our `is_object(structured)` guard runs first. Use `merge!` (infallible variant) — safe because both arguments are guaranteed objects in this code path.

5. **Vector's `.message` must be mapped to VL's `._msg`.** Vector's `kubernetes_logs` source names the log body `.message`, but VictoriaLogs's convention is `._msg`. Without mapping, every event in VL has `_msg: "missing _msg field..."` (a warning string) and the actual body lives in `.message`. The fix is a 4-line VRL addition: `if exists(.message) { ._msg = .message }`.

6. **podMonitor (not serviceMonitor).** Vector chart 0.52.0 only ships a `podMonitor` template, not a `serviceMonitor`. The plan's `serviceMonitor.enabled: true` becomes `podMonitor.enabled: true`. VMAgent discovers both equally well via prometheus-operator-CRDs compat.

7. **Vector customConfig must include `internal_metrics + prometheus_exporter` if podMonitor is enabled.** The chart's default `agent.yaml` declares a `prom-exporter` container port (9090) backed by these components. When `customConfig` replaces the default, we have to re-add them — otherwise the PodMonitor scrapes a port that doesn't exist.

8. **VRL inside the chart's `customConfig` requires `{{ print "{{...}}" }}` escaping.** The chart passes `customConfig` through Helm's `tpl`, which would interpret VRL syntax like `{{ kubernetes.pod_namespace }}` as a Go-template function call. Escape via `{{ print "{{kubernetes.pod_namespace}}" }}` so Helm renders literal `{{kubernetes.pod_namespace}}` for vector to interpret at runtime.

The code blocks in the task body below have been updated to reflect each of these corrections.

---

**Goal:** Ship every Kubernetes container's stdout/stderr to a VictoriaLogs single-node instance via a vector DaemonSet, and expose it as a Grafana datasource using VictoriaMetrics's official Grafana plugin (`victoriametrics-logs-datasource`) — yielding cross-pod, 30-day-retention log search in Grafana Explore.

**Architecture:** Two new Helm wrappers in the existing `observability` ApplicationSet tier (vl-system, vector) plus modifications to the existing grafana wrapper (add plugin to chart values + add new datasource ConfigMap). vlsingle is a StatefulSet (1 replica, 30 Gi local-path PVC, 30d retention). Vector is a workers-only DaemonSet that tails `/var/log/pods/`, runs a `remap` transform that auto-parses JSON-when-detected (and maps `.message` to VL's `._msg` convention), and ships via Loki push protocol at `/insert/loki/api/v1/push`. Grafana queries through the VL plugin which speaks native LogsQL.
...
After this merges, vector (Task 2) ships logs to `http://vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428/insert/loki/api/v1/push`.
...
- [ ] After merge: `kubectl -n vl-system get pods` shows `vl-system-victoria-logs-single-server-0  1/1  Running` (NOT just "Application Synced")
...
# Expect: vl-system-victoria-logs-single-server-0  1/1  Running, restart count 0
...
kubectl -n vl-system port-forward svc/vl-system-victoria-logs-single-server 9428:9428 >/dev/null 2>&1 &
PF=$!
sleep 3
...
## Pre-req
- Task 1 (vl-system) MUST be merged + verified before this merges. Vector ships to `http://vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428` and will retry-with-backoff until vlsingle is reachable.
  ...
  vlsingle.yaml: |
    ...
    datasources:
      - name: VictoriaLogs
        type: loki
        access: proxy
        url: http://vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428
        isDefault: false
        editable: false
```
...
3. **Type / path consistency:**
   - Service URL `http://vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428` referenced in Task 2.7 (vector's loki sink) and Task 3.3 (grafana datasource) — identical. Used identically in spec.
