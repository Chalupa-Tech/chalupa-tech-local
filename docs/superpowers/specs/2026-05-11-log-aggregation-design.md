...
Sub-project #5b closes the log half: ship every container's stdout/stderr to a VictoriaLogs single-node instance, with retention matching metrics (30 days) and a Grafana datasource via VictoriaMetrics's official Grafana plugin (`victoriametrics-logs-datasource`). After #5b, the typical debug workflow becomes "filter by `namespace:argocd AND _msg:error` in Grafana Explore" rather than `kubectl logs argocd-server-... --since=1h | grep error` repeated per pod.
...
5b. **Log aggregation** *(this spec)* — VictoriaLogs + vector log shipper + Grafana datasource via VL's official Grafana plugin (`victoriametrics-logs-datasource`).
...
- Grafana datasource added via the existing `gitops/apps/observability/grafana/` wrapper. Uses VictoriaMetrics's official Grafana plugin (`victoriametrics-logs-datasource`) installed via the chart's `plugins:` values list; datasource is registered through a sidecar-loaded ConfigMap. **Why the plugin (not Grafana's built-in Loki datasource):** VL v1.50.0 serves Loki-compatible *ingestion* (`/insert/loki/api/v1/push`, used by vector) but has NO Loki query API — every `/loki/api/v1/*` and `/select/loki/api/v1/*` path returns HTTP 400 `"unsupported path"`. Queries are native LogsQL only. The plugin speaks LogsQL.
...
- ~~**Built-in Loki datasource (no plugin install)**~~ — the original decision (e) of brainstorming assumed VL serves Loki query API. **Verified false during implementation.** VL has no Loki query API; the VL Grafana plugin (`victoriametrics-logs-datasource`) is what landed. Loki's LogQL syntax does not apply — use native LogsQL (e.g., `namespace:argocd AND _msg:error`).
...
vector DaemonSet (one pod per worker)
    ...
    │  sink: loki — VL's /insert/loki/api/v1/push (Loki wire protocol; INGEST only — VL has no Loki query API)
    │  + VRL transform also maps .message → ._msg so VL recognizes the log body
    ▼
vlsingle StatefulSet (1 replica) at vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428
    │  receives Loki push API at /insert/loki/api/v1/push
    │  compresses + indexes + stores in 30 Gi local-path PVC
    │  serves /select/logsql/{query,tail,...} for native LogsQL queries
    │  (NO Loki query API — every /loki/api/v1/* path returns 400 "unsupported")
    ▼
Grafana
    │  "victoriametrics-logs-datasource" type (plugin installed via chart's plugins: list)
    │  appears as "VictoriaLogs" in the datasource picker alongside "VictoriaMetrics"
    │  full Logs panel + Explore UI work natively against the plugin's LogsQL backend
```
...
vector:
  ...
  customConfig:
    sources:
      ...
      internal_metrics:               # exposes vector's own metrics on :9090
      ...
      parse_json:
        ...
        source: |
          ...
          if err == null && is_object(structured) {
            . = merge!(., structured)   # merge!, not merge — VRL fallibility
          }
          # VictoriaLogs convention: _msg holds the log body. The
          # kubernetes_logs source names the body .message; copy to ._msg
          # so VL doesn't emit "missing _msg field" on every event.
          if exists(.message) {
            ._msg = .message
          }
    ...
    sinks:
      vl:
        type: loki
        inputs: [parse_json]
        # NOTE: VictoriaLogs serves Loki INGEST at /insert/loki/api/v1/push
        # (NOT the standard /loki/api/v1/push, which returns 400). Vector
        # appends /loki/api/v1/push to this endpoint, so adding /insert here
        # produces the correct full URL.
        endpoint: http://vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428/insert
        labels:
          # vector chart's tpl-processed customConfig means VRL syntax must
          # be escaped via `{{ print "{{...}}" }}` to survive Helm template
          # evaluation. See chart's CI test values for the pattern.
      ...
      prom_exporter:                    # required because podMonitor.enabled: true
        type: prometheus_exporter        # scrapes the chart's standard prom-exporter
        inputs: [internal_metrics]       # port. Without this sink, that port doesn't
        address: 0.0.0.0:9090            # exist and VMAgent gets "port not found".
```
...
**Two changes to the existing grafana wrapper:**

1. Add the VL plugin to grafana's `values.yaml` so the chart installs it on pod startup:
...
2. Add a new ConfigMap at `gitops/apps/observability/grafana/templates/datasource-vlsingle.yaml`, mirroring the existing `datasource-vmsingle.yaml`:
  ...
  vlsingle.yaml: |
    ...
    datasources:
      - name: VictoriaLogs
        ...
        access: proxy
        # The VL Grafana plugin speaks native LogsQL; it knows VL's path
        # structure (/select/logsql/* for queries), so the base URL is all
        # that's needed here. Do NOT add a /select or /insert suffix.
        url: http://vl-system-victoria-logs-single-server.vl-system.svc.cluster.local:9428
        isDefault: false
        editable: false
```

The Grafana sidecar (`sidecar.datasources.enabled: true`, label `grafana_datasource: "1"` — already configured from #5) auto-mounts the ConfigMap on next reconcile. The plugin install requires a Grafana pod restart (which the chart triggers automatically when `plugins:` changes).

After Task 3 merges, you'll see "VictoriaLogs" in Grafana Explore alongside "VictoriaMetrics". Queries use **native LogsQL** syntax (e.g., `namespace:argocd AND _msg:error`), NOT Loki's LogQL.
...
# Expect: vl-system-victoria-logs-single-server-0  1/1  Running

kubectl -n vl-system get pvc
# Expect: 30 Gi PVC Bound

kubectl -n vl-system port-forward svc/vl-system-victoria-logs-single-server 9428:9428 &
PF=$!
sleep 2
