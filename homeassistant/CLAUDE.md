...
# Then sudo-mv each into place in one round trip:
...
# 3. Verify via API (no need to ping the user)
TOK="${HOMEASSISTANT_TOKEN:-$(cat ~/.config/ha/llat)}"
...
deploy is healthy. No user roundtrip needed for routine validation.
